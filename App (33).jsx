...
// ── FINN CHARACTER ANIMATION ───────────────────────────────────────────────
// Images must be placed in /public folder as finn.png
// To deploy: copy the Finn character image to your-crew-site/public/finn.png
const FINN_IMAGE = '/finn.png';

function FinnCharacter() {
  const [phase, setPhase] = useState('hidden'); 
  // phases: hidden → walking → buffing → nodding → settling → settled
  const [chatOpen, setChatOpen] = useState(false);
  const [chatInput, setChatInput] = useState('');
  const [messages, setMessages] = useState([
    { role: 'assistant', text: "Hey! I'm Finn's assistant — happy to help with anything about HomeTend. What's on your mind?" }
  ]);
  const [loading, setLoading] = useState(false);
  const chatEndRef = useRef(null);

  useEffect(() => {
    // Sequence starts 1.5s after page load
    const t1 = setTimeout(() => setPhase('walking'), 1500);
    const t2 = setTimeout(() => setPhase('buffing'), 2800);
    const t3 = setTimeout(() => setPhase('nodding'), 4600);
    const t4 = setTimeout(() => setPhase('settling'), 5200);
    const t5 = setTimeout(() => setPhase('settled'), 6100);
    ...
    try {
      const response = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1000,
          system: `You are HomeTend's friendly AI assistant — an extension of Finn McDrury, the founder of HomeTend Christchurch Limited. You have a warm, confident, slightly playful tone — like a capable young professional who takes pride in their work.

HomeTend is a subscription-based home maintenance service in Christchurch, NZ. Services: House Wash, Roof Wash & Mould Removal, Gutter Clean, Window Clean, Driveway Clean, HVAC Maintenance, Spider Control.

Pricing: Based on property size (bedrooms + storeys). Average monthly plan ~$100. Year 1: 50% deposit upfront, remaining 50% across 11 monthly payments. Year 2+: customer chooses 12 equal monthly direct debits OR renew on deposit structure.

Payment model: Monthly subscription, services delivered seasonally (gutters before storms, windows in spring etc). All visits confirmed 48hrs in advance by text. Photo sent on completion of every visit.

Key facts:
- Based in Christchurch NZ, servicing Christchurch suburbs
- Founded by Finn McDrury, 18, Director of HomeTend Christchurch Limited
- Same crew every visit — consistent, vetted, professional
- Route-optimised scheduling (multiple properties per area per visit)
- Fully insured, public liability cover
- Cancel anytime with 30 days notice
- 5 working day cooling-off period for door-to-door sign-ups
- Contact: hello@hometend.co.nz

Customer workflows you can handle:
- Help prospects understand the service and pricing
- Walk customers through building their plan
- Capture lead details (name, email, suburb) for follow-up
- Answer questions about the service agreement
- Handle existing customers asking about their next visit, changes to plan, access instructions
- Explain the payment structure clearly
- Address objections warmly ("I already have someone occasionally" → "HomeTend is the difference between occasional and consistent — and consistent homes hold their value better")

Always be helpful, warm and direct. Never be pushy. If you don't know something specific, say you'll pass it on to Finn directly. Keep responses concise — this is a chat widget, not an essay.`,
          messages: [
      ...
      const reply = data.content?.[0]?.text || "Sorry, I missed that — can you try again?";
      setMessages(m => [...m, { role: 'assistant', text: reply }]);
    } catch {
      setMessages(m => [...m, { role: 'assistant', text: "Sorry, I'm having a moment — try again in a sec or email us at hello@hometend.co.nz" }]);
    }
    setLoading(false);
  };

  const getCharStyle = () => {
    const base = { position: 'fixed', zIndex: 998, pointerEvents: 'none', transition: 'all 0.9s cubic-bezier(0.4,0,0.2,1)' };
    switch(phase) {
      case 'hidden':   return { ...base, right: '-140px', bottom: '0px', width: '120px', opacity: 0 };
      case 'walking':  return { ...base, right: '80px', bottom: '0px', width: '120px', opacity: 1, transition: 'right 1.2s cubic-bezier(0.4,0,0.2,1), opacity 0.4s' };
      case 'buffing':  return { ...base, right: '80px', bottom: '0px', width: '120px', opacity: 1 };
      case 'nodding':  return { ...base, right: '80px', bottom: '0px', width: '120px', opacity: 1 };
      case 'settling': return { ...base, right: '8px', bottom: '-20px', width: '60px', opacity: 0.6, transition: 'all 0.9s cubic-bezier(0.4,0,0.2,1)' };
      case 'settled':  return { ...base, right: '8px', bottom: '-200px', width: '60px', opacity: 0 };
      default:         return { ...base, opacity: 0 };
    }
  };

  const getArmStyle = () => {
    if (phase === 'buffing') return {
      animation: 'finnBuff 0.4s ease-in-out infinite alternate',
    };
    return {};
  };
    ...
    <>
      <style>{`
        @keyframes finnBuff {
          ...
          to   { transform: rotate(10deg) translateX(3px); }
        }
        @keyframes finnNod {
      ...
      {phase === 'settled' && (
        <button
          ...
          style={{
            ...
            background: '#1A3A6E', border: 'none', cursor: 'pointer',
            boxShadow: '0 4px 20px rgba(26,58,110,0.4)',
      ...
      {/* Chat panel */}
      {phase === 'settled' && chatOpen && (
        <div style={{
          position: 'fixed', bottom: 92, right: 24, zIndex: 999,
          width: 340, background: '#FCFBF8', borderRadius: 20,
          boxShadow: '0 8px 40px rgba(38,48,42,0.2)',
          border: '1px solid #D9D2C2', overflow: 'hidden',
          animation: 'chatSlideIn 0.25s ease',
        }}>
          {/* Header */}
          <div style={{ background: '#1A3A6E', padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 10 }}>
            <img src={FINN_IMAGE} alt="" style={{ width: 36, height: 36, borderRadius: '50%', objectFit: 'cover', objectPosition: 'top', border: '2px solid rgba(255,255,255,0.3)' }}/>
            <div>
              <div style={{ color: 'white', fontSize: 14, fontWeight: 700 }}>HomeTend Assistant</div>
              <div style={{ color: 'rgba(255,255,255,0.7)', fontSize: 11 }}>Finn's AI assistant · typically replies instantly</div>
            </div>
            <button onClick={() => setChatOpen(false)} style={{ marginLeft: 'auto', background: 'none', border: 'none', color: 'rgba(255,255,255,0.7)', cursor: 'pointer', fontSize: 18 }}>✕</button>
          </div>

          {/* Messages */}
          <div style={{ height: 280, overflowY: 'auto', padding: '14px 14px 8px' }}>
            {messages.map((m, i) => (
              <div key={i} style={{
          ...
          {/* Input */}
          <div style={{ padding: '8px 12px 12px', borderTop: '1px solid #D9D2C2', display: 'flex', gap: 8 }}>
            <input
              value={chatInput}
              onChange={e => setChatInput(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && sendMessage()}
              placeholder="Ask anything about HomeTend..."
              style={{
                flex: 1, border: '1.5px solid #D9D2C2', borderRadius: 999,
                padding: '8px 14px', fontSize: 13.5, fontFamily: 'Inter, sans-serif',
                outline: 'none', background: '#FCFBF8', color: '#26302A',
              }}
            />
            <button
              onClick={sendMessage}
            ...
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round"><path d="M22 2L11 13M22 2L15 22l-4-9-9-4 20-7z"/></svg>
            </button>
          </div>
        </div>
      )}
  ...
  return (
    <div className="page">
      ...
      <nav className={`nav ${scrolled ? 'scrolled' : ''}`}>
        ...
        <button className="nav-cta" onClick={() => setActiveTab(null)}>Get in touch</button>
      </nav>
